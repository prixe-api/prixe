...
ws_url = "wss://ws.prixe.io/ws?api_key=REDACTED"
...
try:
    while True:
        time.sleep(2)  # Changed to 2 seconds for more frequent heartbeat logs
        ws.send(json.dumps(subscribe_message))
        counter += 1
        print(f"[{get_timestamp()}] 💓 HEARTBEAT #{counter} - WebSocket thread alive: {wst.is_alive()}")
